...
@section('content')
        ...
        <div class="bg-gray-50 border border-gray-200 rounded mt-4 p-6 pt-8">
            <div class="flex">
                <div class="flex flex-col items-center p-6 space-y-6 rounded-lg bg-veryLightGray md:flex  md:w-1/3">
                    <h3 class="text-lg font-bold">{{ $collaborate->title }}</h3>
    ...
    <form method="POST" action="{{ route('collaborate.destroy', $collaborate->id) }}">
        ...
        @method('DELETE')
        <div class="flex ml-8">
            <a class="px-24 py-2 mt-4 ml-2 text-white bg-blue-900 rounded-lg hover:bg-red-500"
                href="{{ route('collaborate.edit', $collaborate->id) }}">Edit</a>
            <button type="submit"
