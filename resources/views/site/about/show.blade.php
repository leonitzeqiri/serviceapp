...
@section('content')
    <a href="/about" class="inline-block text-black ml-8 mb-2 mt-8"><i class="fa-solid fa-arrow-left"></i> Back
    </a>
...
<form method="POST" action="{{ route('about.destroy', $about->id) }}">
    ...
    <form method="POST" action="{{ route('about.destroy', $about->id) }}">
        ...
        <div class="flex ml-8">
            <a
                class="px-24 py-2 mt-4 ml-2 text-white bg-blue-900 rounded-lg hover:bg-red-500"href="{{ route('about.edit', $about->id) }}">Edit</a>
            <button type="submit"
                class="px-24 py-2 mt-4 ml-2 text-white bg-red-900 rounded-lg hover:bg-red-500">Delete</button>
        </div>
    @endsection
